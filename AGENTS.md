...
|------|-----------------|----------------|
| CLI commands and flags | `printUsage` in `cmd/soma/main.go` | README "Commands" |
| Config keys | `internal/config/config.go` (structs + template text) | README "Configuration", `printUsage` example |
| Keyboard controls | keymap in `internal/app/update.go` | README "Keyboard Controls" (`<kbd>` tables) |
